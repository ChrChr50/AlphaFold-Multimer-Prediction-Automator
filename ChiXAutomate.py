...
class DirHandler(FileSystemEventHandler):
    ...
    def on_created(self, event: DirCreatedEvent):
        ...
        run(session, 'movie encode' + VideoFile_Path)
        new_video = client.folder(Box_VideoFolder_ID).upload(VideoFile_Path + 'movie' + file_num + '.mp4')
        if third:
    ...
    def on_moved(self, event: DirMovedEvent):
        new_folder = client.folder(Box_ModelFolder_ID).upload(newdir)
